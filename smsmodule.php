...
//SMS via GSM Modem - A PHP class to send SMS messages via a GSM modem attached to the computer's serial port.
...
$_SQL=mysqli_query($con,"SELECT * FROM tblport");
...
$gsm_send_sms->port = $_PORT;
$gsm_send_sms->baud = 19200;
$gsm_send_sms->init();

//Get Mobile number from the receiver
/*include("dbstring.php");
@$Receiver_Mobile="";
$_SQLMaster=mysqli_query($con,"SELECT * FROM tblsmsalert WHERE smsenabled=1");
if($rowm=mysqli_fetch_array($_SQLMaster,MYSQLI_ASSOC)){
$Receiver_Mobile=$rowm['mobile'];
}
else{
 $_Error=mysqli_error($con);
echo "<div style='color:red'>Failed to get SMS Status</div>";
}
*/

//$status = $gsm_send_sms->send('0242602522', 'Hi');

$status = $gsm_send_sms->send($_Mobile, $Exam_Results);
