			...
			<td width="100%">
				...
				<table>
					<tr>
						...
						<form method="post">
						<button class="button-print" name="print_all_reports"><i class="fa fa-print"></i> Print All Subjects</button>							
						</form>
						</td>
					</tr>
				</table>
				...
				echo "<thead><th>SUBJECT ID</th><th>SUBJECT</th></thead>";
				echo "<tbody>";
				while($row_v=mysqli_fetch_array($_SQL_EXECUTE_VIEW,MYSQLI_ASSOC))
				{
				$_SQL_EXECUTE=mysqli_query($con,"SELECT *,sc.datetimeentry FROM tblsubjectclassification sc
				...
				//echo "<td align='center' width='10%'><a title='Delete $row[subject]' onclick=\"javascript:return confirm('Do you want to delete $row[subject]?');\" href='view-subject-classified.php?delete_subject=$row[classificationid]'<i class='fa fa-trash-o' style='color:red'></i></a></td>";
				
				//echo "</td>";
				echo "<td align='left' width='15%'>$row[subjectid]</td>";
